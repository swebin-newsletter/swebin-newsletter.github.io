import{s as C,i as B,e as O,f as q,a as E,p as x,I as H,G as N,b as c,c as A,o as G,d as o,g as D,l as F,h as L,j as U,v as W,k as K}from"./escape-html.BJ-CLDPy.js";function J(e,n,t){const s=O(e);return{issue_id:e.issue_id,title:e.subject,public_url:`${n}/issues/${s}/`,publication_status:e.status,published_at:e.published_at,updated_at:e.published_at||e.publish_date,...t?{thumbnail:t}:{},source_count:e.articles.length}}async function Q(e,n,t){const s=C(e.filter(a=>B(a.status)));return Promise.all(s.map(async a=>J(a,n,await t(a))))}const v=452;function V(e){let n;try{n=JSON.parse(e)}catch{return null}if(!n||typeof n!="object")return null;const t=n,s=Number(t.last_completed_page??0);return Number.isFinite(s)?{lastCompletedPage:s,updatedAt:typeof t.updated_at=="string"?t.updated_at:"",completedCount:Array.isArray(t.completed_source_ids)?t.completed_source_ids.length:0,failedCount:Array.isArray(t.failed_source_ids)?t.failed_source_ids.length:0,failedPagesCount:Array.isArray(t.failed_pages)?t.failed_pages.length:0,targetPage:v,percent:Math.min(100,Math.round(s/v*100)),complete:s>=v}:null}function Y(e,n){if(n===null||!Number.isFinite(n))return null;const t=e-n;return t>0?t:null}async function z(e,n,t,s){const a=await E(e,`content/issues/${n}/index.md`,t,s),{data:i}=x(a),r=H.safeParse(i);return r.success?r.data:(console.error(`manage-data: skipping unparseable issue "${n}"`,r.error.message),null)}async function Z(e,n,t){try{const s=await E(e,"data/crawl-checkpoint.json",n,t);return V(s)}catch(s){if(s instanceof N&&s.status===404)return null;throw s}}async function X(e,n,t,s=fetch){const a=await q(e,"content/issues",n,s),[i,r]=await Promise.all([Promise.all(a.map(b=>z(e,b,n,s))),Z(e,n,s)]),p=i.filter(b=>b!==null),h=C(p),l=await Q(p,t,async()=>{});return{crawlProgress:r,allIssues:h,publishedIssues:l}}const m=20,ee=[{value:"id_desc",label:"최신 호 순"},{value:"id_asc",label:"오래된 호 순"},{value:"status",label:"상태순"},{value:"subject",label:"제목순"}];function T(e,n){const t=[...e];switch(n){case"id_desc":t.sort((s,a)=>a.issue_id.localeCompare(s.issue_id));break;case"id_asc":t.sort((s,a)=>s.issue_id.localeCompare(a.issue_id));break;case"status":t.sort((s,a)=>s.status.localeCompare(a.status)||a.issue_id.localeCompare(s.issue_id));break;case"subject":t.sort((s,a)=>s.subject.localeCompare(a.subject,"ko"));break}return t}function te(e,n){const t=n.trim().toLowerCase();return t===""?e:e.filter(s=>s.issue_id.toLowerCase().includes(t)||s.subject.toLowerCase().includes(t))}const se={published:{label:"공개 중",tone:"success"},unpublished:{label:"비공개",tone:"warning"}},ne=[{icon:"📝",title:"콘텐츠 관리",body:"기사 편집(제목·요약·썸네일), 뉴스레터 호 작성과 기사 구성, 발행 상태 관리는 모두 /admin/에서 이루어집니다. GitHub 계정으로 로그인해야 하며, 해당 저장소에 접근 권한이 있는 계정만 실제로 내용을 보거나 수정할 수 있습니다.",href:"/admin/",label:"/admin/ 열기"},{icon:"👥",title:"수신인 · 발신인",body:"뉴스레터를 받을 사람 추가/삭제는 /admin-recipients/에서 관리합니다. 테스트 수신과 실제 발송 수신은 서로 다른 플래그로 독립적으로 관리되며, SMTP 비밀번호 등 발신 계정의 민감 정보는 여기서 다루지 않습니다.",href:"/admin-recipients/",label:"/admin-recipients/ 열기"}];function I(e){return e?e.slice(0,10):"-"}function ae(e){return e.public_slug.trim()||e.issue_id}function ie(e,n=null){const t=e.crawlProgress;let s;if(!t)s='<span class="stat-value stat-value-muted">기록 없음</span>';else if(t.complete){const a=n?` <span class="stat-delta">+${n}</span>`:"";s=`<span class="stat-value">총 게시물 ${t.completedCount}개${a}</span>
    ...
    <section>
      ...
      </li>`}function P(e,n){return e.length===0?'<p class="empty" id="issue-preview-empty">검색 결과가 없습니다.</p>':`<ul class="issue-table">${e.map(t=>oe(t,n)).join("")}</ul>`}function R(e,n){if(n<=1)return"";const t=Array.from({length:n},(s,a)=>a+1).map(s=>`<button type="button" class="pagination-link${s===e?" active":""}" data-page="${s}" aria-current="${s===e?"page":"false"}">${s}</button>`).join("");return`
    <nav class="pagination" aria-label="미리보기 목록 페이지 이동">
      ...
      </section>`;const t=T(e.allIssues,"id_desc").slice(0,m),s=Math.ceil(e.allIssues.length/m);return`
    <section>
      <h2>전체 뉴스레터 호 미리보기</h2>
      <p>
      ...
      <div id="issue-preview-list">${P(t,n)}</div>
      <div id="issue-preview-pagination">${R(1,s)}</div>
...
`)}const be=A(["crawl_recent","crawl_article","build_example_issue","publish_issue","unpublish_issue","redeploy_site","send_test"]),he=A(["requested","processing","completed","failed","cancelled"]),me=G({request_id:o().min(1),action:be,target:o().default(""),status:he,requested_by:o().default(""),requested_at:o().min(1),confirmation:o().default(""),result_summary:o().default(""),result_url:o().default(""),completed_at:o().default("")});function fe(e,n=new Date,t=Math.random().toString(36).slice(2,8)){const s=`op-manage-crawl-recent-${n.getTime()}-${t}`;return me.parse({request_id:s,action:"crawl_recent",target:"",status:"requested",requested_by:e,requested_at:n.toISOString(),confirmation:"",result_summary:"",result_url:"",completed_at:""})}async function ge(e,n,t,s=fetch){const a=`content/operations/${n.request_id}.json`,i=`${JSON.stringify(n,null,2)}
`;await D(e,a,i,`chore(operations): request ${n.action} via /manage/`,t,s)}const k="swebin-manage-last-completed-count";function we(){try{const e=window.localStorage.getItem(k);if(e===null)return null;const n=Number(e);return Number.isFinite(n)?n:null}catch{return null}}function ve(e){try{window.localStorage.setItem(k,String(e))}catch{}}const _e="https://swebin-sveltia-auth.swebin-newsletter.workers.dev",_={owner:"swebin-dev",repo:"swebin-newsletter",ref:"main"},ye="swebin.newsletter@gmail.com",w=document.getElementById("manage-status"),y=document.getElementById("manage-dashboard"),u=document.getElementById("login-btn"),f=document.getElementById("logout-btn"),g=document.getElementById("account-label");function d(e){w.innerHTML="";const n=document.createElement("p");n.textContent=e,w.appendChild(n)}function $e(e,n){const t=document.getElementById("issue-preview-search"),s=document.getElementById("issue-preview-sort"),a=document.getElementById("issue-preview-list"),i=document.getElementById("issue-preview-pagination");if(!t||!s||!a||!i)return;let r=1;function p(){const h=te(e,t.value),l=T(h,s.value),b=Math.max(1,Math.ceil(l.length/m));r=Math.min(r,b);const $=(r-1)*m,j=l.slice($,$+m);a.innerHTML=P(j,n),i.innerHTML=R(r,b)}t.addEventListener("input",()=>{r=1,p()}),s.addEventListener("change",()=>{r=1,p()}),i.addEventListener("click",h=>{const l=h.target.closest("button[data-page]");!l||l.classList.contains("disabled")||(r=Number(l.getAttribute("data-page")),p())})}function Ie(e,n){const t=document.getElementById("crawl-recent-btn"),s=document.getElementById("crawl-recent-status"),a=t?.querySelector(".btn-spinner");!t||!s||t.addEventListener("click",async()=>{t.disabled=!0,a&&(a.hidden=!1),s.textContent="요청을 제출하는 중...";try{const i=fe(n??"manage-dashboard");await ge(_,i,e),s.textContent="요청을 제출했습니다. GitHub Actions가 자동으로 처리하며 보통 몇 분 걸립니다 - 잠시 후 이 페이지를 새로고침해서 확인해 주세요."}catch{s.textContent="요청 제출에 실패했습니다. 잠시 후 다시 시도해 주세요."}finally{t.disabled=!1,a&&(a.hidden=!0)}})}async function M(e){if(d("불러오는 중입니다..."),!await W(_,e)){L(),u.hidden=!1,f.hidden=!0,g.hidden=!0,d("이 저장소에 대한 접근 권한이 없는 계정입니다. 권한이 있는 계정으로 다시 로그인해 주세요.");return}const t=await K(e);t&&(g.textContent=`${t}(으)로 로그인됨`,g.hidden=!1);try{const s=await X(_,e,window.location.origin),a=we(),i=s.crawlProgress?Y(s.crawlProgress.completedCount,a):null;s.crawlProgress&&ve(s.crawlProgress.completedCount),y.innerHTML=pe(s,window.location.origin,ye,i),$e(s.allIssues,window.location.origin),Ie(e,t),y.hidden=!1,u.hidden=!0,f.hidden=!1,d(""),w.hidden=!0}catch{d("불러오는 중 오류가 발생했습니다. 다시 시도해 주세요.")}}u.addEventListener("click",async()=>{u.disabled=!0,d("로그인 창을 여는 중입니다...");try{const e=await F({workerBaseUrl:_e});await M(e)}catch(e){d(e instanceof Error?e.message:"로그인에 실패했습니다.")}finally{u.disabled=!1}});f.addEventListener("click",()=>{L(),y.hidden=!0,u.hidden=!1,f.hidden=!0,g.hidden=!0,w.hidden=!1,d("로그아웃했습니다. 다시 보려면 로그인해 주세요.")});const S=U();S&&(f.hidden=!1,u.hidden=!0,M(S));
